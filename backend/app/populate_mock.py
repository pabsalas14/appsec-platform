...
async def populate():
    async with SessionLocal() as db:
        ...
        if not repo_obj:
            repo_obj = Repositorio(
                id=uuid.uuid4(), celula_id=uuid.uuid4(), nombre="repo-mock", url="http://git", activo=True
            )
            db.add(repo_obj)
        if not servicio_obj:
            ...
            db.add(servicio_obj)
        if not activo_obj:
            activo_obj = ActivoWeb(
                id=uuid.uuid4(), celula_id=uuid.uuid4(), url="https://mock.com", nombre="mock-web", activo=True
            )
            db.add(activo_obj)
        await db.commit()
