...
async def list_rules(
    ...
    filters: list = [AIRule.deleted_at.is_(None)]

    if search:
        filters.append(AIRule.name.ilike(f"%{search}%"))
    ...
    if action_type:
        ...
        filters.append(AIRule.action_type == action_type)

    if enabled is not None:
        filters.append(AIRule.enabled == enabled)

    query = select(AIRule).where(*filters).order_by(AIRule.created_at.desc()).offset(skip).limit(limit)
    rows = (await db.execute(query)).scalars().all()
    total = await db.scalar(select(func.count()).select_from(AIRule).where(*filters))
    total_i = int(total or 0)
    page = (skip // limit) + 1

    logger.info("ai_rule.list", extra={"skip": skip, "limit": limit, "total": total_i})
...
async def create_rule(
    ...
    rule = await ai_rule_svc.create(
        db,
        payload,
        extra={"created_by": admin.id},
    )
    await audit_record(
        db,
        action="ai_rule.create",
        entity_type="ai_rules",
        entity_id=str(rule.id),
        actor_id=admin.id,
    )
    logger.info(
        "ai_rule.create",
        extra={"rule_id": str(rule.id), "trigger_type": payload.trigger_type, "action_type": payload.action_type},
    )
...
async def delete_rule(
    ...
    await ai_rule_svc.delete(db, rule_id, actor_id=admin.id)
    await audit_record(
        db,
        action="ai_rule.delete",
        entity_type="ai_rules",
        entity_id=str(rule_id),
        actor_id=admin.id,
    )
...
async def test_rule(
    ...
    try:
        ...
        result = AIRuleTestResult(
            rule_id=rule_id,
            status="success",
            message=(
                f"Test executed successfully. Rule would trigger on {rule.trigger_type} and execute {rule.action_type}."
            ),
            dry_run=True,
            execution_time_ms=execution_time_ms,
        )
