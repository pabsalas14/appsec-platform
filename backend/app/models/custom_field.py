...
from sqlalchemy import DateTime, String, Text, text, Integer
...
class CustomField(SoftDeleteMixin, Base):
    ...
    field_type: Mapped[str] = mapped_column(String(50), nullable=False)  # text, number, date, select, boolean, url, user_ref
...
class CustomFieldValue(SoftDeleteMixin, Base):
    ...
    entity_type: Mapped[str] = mapped_column(String(100), nullable=False, index=True)
    entity_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False, index=True)  # ID of vulnerabilidad, iniciativa, etc
