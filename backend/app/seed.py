...
async def _seed_admin(db) -> User:
    ...
    if existing:
        logger.info("✅ Admin user already exists (id=%s)", existing.id)
        return existing
...
async def _seed_settings(db) -> None:
    ...
    for row in DEFAULT_SETTINGS:
        ...
        if result.rowcount:
            count += 1
    await db.flush()
    logger.info("🌱 System settings: %d new keys seeded (%d total defined)", count, len(DEFAULT_SETTINGS))
...
async def _seed_regla_sods(db, admin_id: uuid.UUID) -> None:
    ...
    for sod in SOD_SEEDS:
        ...
        if result.rowcount:
            count += 1
    await db.flush()
    logger.info("🌱 ReglaSoD: %d new rules seeded", count)
...
async def _seed_tipos_prueba(db, admin_id: uuid.UUID) -> None:
    ...
    for tp in TIPO_PRUEBA_SEEDS:
        ...
        count += 1
    await db.flush()
    logger.info("🌱 TipoPrueba: %d new types seeded", count)
...
async def seed() -> None:
    ...
    logger.info("━━━ AppSec Platform — Seed Start ━━━")
