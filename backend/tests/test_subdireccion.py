"""Smoke + IDOR tests for the subdireccion entity — generated."""
...
@pytest.mark.asyncio
async def test_create_subdireccion(client: AsyncClient, auth_headers: dict):
...
@pytest.mark.asyncio
async def test_subdireccion_requires_auth(client: AsyncClient):
    assert (await client.get(BASE_URL)).status_code == 401
    assert (await client.get(f"{BASE_URL}/{uuid4()}")).status_code == 401


@pytest.mark.asyncio
async def test_subdireccion_idor_protected(
    client: AsyncClient,
    auth_headers: dict,
