...
async def test_aprobar_aceptacion_requires_permission(
    ...
    denied = await client.post(f"{BASE_URL}/{rid}/aprobar", headers=auth_headers, json={"notas": "ok"})
...
@pytest.mark.asyncio
async def test_aceptacion_export_requires_permission(client: AsyncClient, readonly_auth_headers: dict[str, str]):
...
@pytest.mark.asyncio
async def test_aceptacion_export_csv_success(client: AsyncClient, admin_auth_headers: dict):
    vuln_id = await _create_vuln(client, admin_auth_headers)
    user_info = await client.get("/api/v1/auth/me", headers=admin_auth_headers)
    assert user_info.status_code == 200, user_info.text
    owner_id = user_info.json()["data"]["id"]
    ...
    created = await client.post(
        ...
        json={
            "vulnerabilidad_id": vuln_id,
            "justificacion_negocio": "Riesgo aceptado temporalmente por impacto residual controlado",
            "propietario_riesgo_id": owner_id,
            "fecha_revision_obligatoria": "2027-11-01T00:00:00Z",
        },
    )
    assert created.status_code == 201, created.text
    ...
    assert "vulnerabilidad_id,propietario_riesgo_id,estado" in resp.text
    assert "Riesgo aceptado temporalmente por impacto residual controlado" in resp.text
