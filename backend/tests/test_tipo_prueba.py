...
async def test_tipo_prueba_valid_categorias(client: AsyncClient, auth_headers: dict, categoria: str):
    payload = {
...
@pytest.mark.asyncio
async def test_tipo_prueba_rejects_invalid_categoria(client: AsyncClient, auth_headers: dict):
    payload = {
        "nombre": "Bad Type",
...
@pytest.mark.asyncio
async def test_tipo_prueba_update_and_soft_delete(client: AsyncClient, auth_headers: dict):
    ...
    # Update nombre
    patch_resp = await client.patch(f"{BASE_URL}/{rid}", headers=auth_headers, json={"nombre": "DAST Scan v2"})
