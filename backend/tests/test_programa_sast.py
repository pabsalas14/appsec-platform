"""Smoke + IDOR tests for the programa_sast entity — generated."""
...
BASE_URL = "/api/v1/programa_sasts"


@pytest.mark.asyncio
async def test_create_programa_sast(client: AsyncClient, auth_headers: dict):
    repo_id = await create_repositorio_id(client, auth_headers)
    SAMPLE_PAYLOAD = {
        "nombre": "Test SAST Program",
        "ano": 2024,
...
@pytest.mark.asyncio
async def test_list_programa_sasts_empty(client: AsyncClient, auth_headers: dict):
...
@pytest.mark.asyncio
async def test_programa_sast_idor_protected(
    client: AsyncClient,
    auth_headers: dict,
