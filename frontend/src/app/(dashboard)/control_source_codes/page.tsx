...
import {
  useControlSourceCodes,
  useCreateControlSourceCode,
  useDeleteControlSourceCode,
  useUpdateControlSourceCode,
} from '@/hooks/useControlSourceCodes';
import { logger } from '@/lib/logger';
import {
  ControlSourceCodeCreateSchema,
  type ControlSourceCode,
  type ControlSourceCodeCreate,
} from '@/lib/schemas/control_source_code.schema';
import { extractErrorMessage, formatDate } from '@/lib/utils';

const formSchema = ControlSourceCodeCreateSchema;
type FormData = z.infer<typeof formSchema>;

function FormFields({ initial, onSuccess }: { initial?: ControlSourceCode | null; onSuccess: () => void }) {
  const createMut = useCreateControlSourceCode();
  const updateMut = useUpdateControlSourceCode();
  ...
  const onSubmit = form.handleSubmit((data) => {
    const payload: ControlSourceCodeCreate = {
    ...
    if (isEdit && initial) {
      updateMut.mutate(
        { id: initial.id, ...payload },
        {
          ...
          onError: (e) => {
            logger.error('control_source_code.update.failed', { id: initial.id, error: e });
      ...
      createMut.mutate(payload, {
        ...
        onError: (e) => {
          logger.error('control_source_code.create.failed', { error: e });
...
export default function ControlSourceCodesPage() {
  const { data: rows, isLoading, isError } = useControlSourceCodes();
  const [q, setQ] = useState('');
  const [createOpen, setCreateOpen] = useState(false);
  const [edit, setEdit] = useState<ControlSourceCode | null>(null);
  const deleteMut = useDeleteControlSourceCode();
  ...
  return (
    <PageWrapper className="space-y-6 p-6">
      <PageHeader title="Controles (código)" description="Controles asociados a revisiones de código.">
                              ...
                              <AlertDialogAction
                                onClick={() =>
                                  deleteMut.mutate(item.id, {
                                    ...
                                    onError: (e) => {
                                      logger.error('control_source_code.delete.failed', { id: item.id, error: e });
