  ...
  return (
    <div className="min-h-screen bg-dashboard-canvas text-dashboard-on-strong p-6 font-sans">
      <div className="flex flex-col gap-4 justify-between items-start mb-8 sm:flex-row sm:items-center">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <DashboardCsvExportButton
            ...
            className="bg-dashboard-elevated border-dashboard-border text-xs h-9"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div className="glass-hover border-b-4 border-slate-500 p-5 rounded-xl bg-dashboard-surface/50">
          <div className="flex items-center gap-2 mb-2 text-slate-400">
            <Layers className="h-4 w-4" />
            <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Total Releases</span>
          </div>
          {kpiLoading ? <div className="h-8 w-12 bg-dashboard-elevated animate-pulse rounded" /> : <div className="text-3xl font-bold">{kpi?.total_releases ?? 0}</div>}
        </div>

        <div className="glass-hover border-b-4 border-amber-500 p-5 rounded-xl bg-dashboard-surface/50">
          ...
          {kpiLoading ? <div className="h-8 w-12 bg-dashboard-elevated animate-pulse rounded" /> : <div className="text-3xl font-bold text-amber-500">{kpi?.pending_approval ?? 0}</div>}
        </div>

        <div className="glass-hover border-b-4 border-blue-500 p-5 rounded-xl bg-dashboard-surface/50">
          <div className="flex items-center gap-2 mb-2 text-blue-500">
            <PlayCircle className="h-4 w-4" />
            <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">En Progreso</span>
          </div>
          {kpiLoading ? <div className="h-8 w-12 bg-dashboard-elevated animate-pulse rounded" /> : <div className="text-3xl font-bold text-blue-500">{kpi?.in_progress ?? 0}</div>}
        </div>

        <div className="glass-hover border-b-4 border-emerald-500 p-5 rounded-xl bg-dashboard-surface/50">
          <div className="flex items-center gap-2 mb-2 text-green-500">
            <CheckCircle2 className="h-4 w-4" />
            <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Completadas</span>
          </div>
          {kpiLoading ? <div className="h-8 w-12 bg-dashboard-elevated animate-pulse rounded" /> : <div className="text-3xl font-bold text-green-500">{kpi?.completed ?? 0}</div>}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        <div className="bg-dashboard-surface border border-dashboard-border rounded-xl overflow-hidden shadow-xl">
          <div className="p-5 border-b border-dashboard-border bg-dashboard-elevated/30">
                  ...
                  <tbody className="divide-y divide-dashboard-border">
                    {tableData?.items.map((item) => (
                      <tr key={item.id} className="hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className="px-5 py-4"><span className="px-2 py-0.5 bg-dashboard-elevated rounded text-xs">v{item.version}</span></td>
