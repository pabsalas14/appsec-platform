...
import {
  useActividadMensualDasts,
  useActividadMensualDastScoringConfig,
  useCreateActividadMensualDast,
  useDeleteActividadMensualDast,
  useUpdateActividadMensualDast,
} from '@/hooks/useActividadMensualDasts';
import { useClientPagedList } from '@/hooks/useClientPagedList';
import { useProgramaDasts } from '@/hooks/useProgramaDasts';
import { logger } from '@/lib/logger';
import {
  ActividadMensualDastCreateSchema,
  type ActividadMensualDast,
  type ActividadMensualDastCreate,
} from '@/lib/schemas/actividad_mensual_dast.schema';
import { extractErrorMessage, formatDate } from '@/lib/utils';

const formSchema = ActividadMensualDastCreateSchema;
...
function FormFields({
  ...
  initial?: ActividadMensualDast | null;
  onSuccess: () => void;
  progOptions: { value: string; label: string }[];
  subEstadoOptions: string[];
}) {
  const createMut = useCreateActividadMensualDast();
  const updateMut = useUpdateActividadMensualDast();
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: initial
      ...
      : {
          programa_dast_id: progOptions[0]?.value ?? '',
  ...
  const onSubmit = form.handleSubmit((data) => {
    const payload: ActividadMensualDastCreate = {
    ...
    if (isEdit && initial) {
      updateMut.mutate(
        { id: initial.id, ...payload },
        {
          ...
          onError: (e) => {
            logger.error('actividad_mensual_dast.update.failed', { error: e });
      ...
      createMut.mutate(payload, {
        ...
        onError: (e) => {
          logger.error('actividad_mensual_dast.create.failed', { error: e });
...
export default function ActividadMensualDastsPage() {
  const { data: rows, isLoading, isError } = useActividadMensualDasts();
  const { data: progs } = useProgramaDasts();
  const { data: scoringCfg } = useActividadMensualDastScoringConfig();
  const [q, setQ] = useState('');
  const [sortDesc, setSortDesc] = useState(true);
  const [createOpen, setCreateOpen] = useState(false);
  const [edit, setEdit] = useState<ActividadMensualDast | null>(null);
  const deleteMut = useDeleteActividadMensualDast();
  ...
  const filtered = useMemo(() => {
    ...
    return list.filter(
      (r) =>
        ...
        progName(r.programa_dast_id).toLowerCase().includes(n) ||
  ...
  return (
    <PageWrapper className="space-y-6 p-6">
      <PageHeader title="Actividad mensual DAST" description="Métricas agregadas por mes y programa DAST.">
        <Dialog open={createOpen} onOpenChange={setCreateOpen}>
          ...
          <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
            <DialogHeader><DialogTitle>Nueva actividad DAST</DialogTitle></DialogHeader>
            <FormFields
        ...
        <CardContent className="p-4">
          ...
          {list.total > 0 && (
            <DataTable>
              <DataTableHead>
                <tr>
                  <DataTableTh>Programa DAST</DataTableTh>
                ...
                {list.paged.map((item) => (
                  <DataTableRow key={item.id}>
                    <DataTableCell className="text-sm text-muted-foreground max-w-[160px] truncate">
                      {progName(item.programa_dast_id)}
                    </DataTableCell>
      ...
      <Dialog open={edit !== null} onOpenChange={(o) => !o && setEdit(null)}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>Editar actividad DAST</DialogTitle></DialogHeader>
          {edit && (
            <FormFields
              key={edit.id}
              initial={edit}
