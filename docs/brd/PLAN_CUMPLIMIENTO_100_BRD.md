...
- **Avance fases A–B (2026-04-25, entrega amplia):** **A2/A3** — además de inventario: **export CSV + plantilla + import** en subdirección, gerencia, organización, célula, servicio, tipo de prueba y control de seguridad (`catalogs.*`, auditoría por import/export). **B1** — score mensual SAST calculado desde `scoring.pesos_severidad` + `sub_estado` + clave admin `scoring.sast_mensual`. **B2** — `POST /actividad_mensual_sasts/{id}/sincronizar-hallazgos`. **B3** — `metadatos_motor` JSON en `programa_sasts` / `programa_dasts`. **B4** — sesión TM: `backlog_tareas`, `plan_trabajo`, `activo_web_secundario_id`. **C2 (P14)** — `scan_id` en pipeline. Pendiente: A1 UI “serio” en todas las pantallas, A4 pruebas end-to-end de herencia org, D1–D2, F2/F3.  
