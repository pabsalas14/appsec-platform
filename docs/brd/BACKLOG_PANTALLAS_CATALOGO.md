...
**Entrega y plan:** `service_releases/` (FK `servicio_id`, estados alineados al backend) e `iniciativas/` (célula opcional, fechas) — CRUD, sección «Entrega y plan (BRD)» en la barra y palette.
...
Incluye (no exhaustivo): `hallazgo_*`, `programa_*`, `control_*`, etc. (excl. jerarquía, inventario §3.2–3.3, `service_releases` e `iniciativas` si ya migrados arriba).
